...
const BlogCard = ({ blog }) => {
  ...
  return (
    <Link to={`/blog/${blog._id}`} className="group">
      <article className="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 h-full flex flex-col">
        {/* Cover Image */}
        <div className="relative h-48 bg-linear-to-br from-indigo-100 to-purple-100 overflow-hidden">
          ...
          {/* Category Badge */}
          <span className={`absolute top-4 left-4 px-3 py-1 rounded-full text-xs font-semibold ${getCategoryColor(blog.category)}`}>
            {blog.category}
          </span>
        </div>
        ...
        <div className="p-5 flex flex-col grow">
          ...
          <div className="flex items-center justify-between pt-4 border-t border-gray-100">
            ...
            <div className="flex items-center gap-3 text-gray-400">
              ...
              >
                <svg className={`w-4 h-4 ${blog.isLiked ? 'fill-current' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
